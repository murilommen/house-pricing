# regression_pipeline.py (KFP pipeline definition)
...
def load_and_preprocess(file_path: str,
    ...
    pd.DataFrame(X_test_processed).to_csv(x_test_output.path + ".csv", index=False)
...
def copy_artifacts_to_gcs(
    ...
    # Construct the destination GCS paths *directly* under pipeline_root/artifacts
    model_filename = model_input.metadata["model_filename"]

    # Get the bucket name and blob prefix from pipeline_root
    bucket_name = pipeline_root.split("/")[2]
    # blob_prefix = "/".join(pipeline_root.split("/")[3:]) + "/artifacts/"
...
@dsl.component(packages_to_install=["pandas", "numpy", "scikit-learn", "gcsfs", "google-cloud-aiplatform"])
def evaluate_and_register(x_test_input: Input[Dataset],
                          y_test_input: Input[Dataset],
                          model_input: Input[Model],
                          preprocessor_input: Input[Artifact],
                          pipeline_root: str,  # Get the pipeline_root
                          model_name: str,
    ...
    def register_model_to_vertex(model_name: str, rmse: float, pipeline_root: str,
        ...
        if not model_registry:
            ...
            try:
                # artifact_uri is now set to pipeline_root
    ...
    # Call register_model_to_vertex, passing only necessary arguments
